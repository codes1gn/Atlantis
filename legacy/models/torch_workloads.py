...
from .baselines.pytorch.mobilenetv2 import MobileNetV2
from .baselines.pytorch.dcgan import DCGAN
from .baselines.pytorch.yolov3 import YoloV3
from .baselines.pytorch.gpt2 import gpt2
from .baselines.pytorch.shufflenetV2 import ShuffleNetV2
from .baselines.pytorch.squeezenet import SqueezeNet
from .baselines.pytorch.vit import ViT
import logging
import numpy as np
...
def get_torch_input_data(name, batch_size):
    ...
    if name == "gpt2":
        input_data = torch.from_numpy(np.random.randint(
            0, 100, size=input_shape).astype("int32"))
    else:
        input_data = torch.from_numpy(
            np.random.uniform(-1, 1, size=input_shape).astype("float32"))
...
def load_torch_model_from_pth(name, batch_size):
    ...
    model = torch.jit.load(
        f"{this_code_path}/../baselines/pytorch/models/{name}.pth")
    model.eval()
...
def get_network_from_torch(name, batch_size):
    ...
    mod, params = relay.frontend.from_pytorch(
        torch_model, shape_arr_tmp)  # , freeze_params=True)
...
# NOTE: Make sure that you executed codes in "baselines/pytorch_new" to have the most recent onnx files
def get_onnx_from_torch(name, batch_size):
    assert name in WORKLOADS_DIC
    # if batch_size > 1, we need to think about how to take care of bert and nasrnn
    # assert batch_size == 1
