...
def resnet_block(input, strides, out_channels, name):
    t = make_conv2d(input_tensor=input, filter_shape=(1, 1, input.shape[1].value, out_channels), strides=(
    ...
    if (strides[2] > 1) or (input.shape[1].value != out_channels * 4):
        input = make_conv2d(input_tensor=input, filter_shape=(
            1, 1, input.shape[1].value, out_channels*4), strides=strides, padding="SAME", actimode="RELU", name=name+"_conv4")
...
parser.add_argument(
    "--iterations", help="How many iterations to average for timing (default 5000)", type=int, default=1000)
parser.add_argument(
    "--discard_iter", help="How many iterations to not time during warm up (default 1000)", type=int, default=1000)
