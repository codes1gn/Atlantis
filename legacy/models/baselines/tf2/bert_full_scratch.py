...
import numpy as np
import time
from shared_functions import make_matmul, measure_tf2_gpu, get_tf2_args
import os
import math
...
class LayerNorm(object):
    ...
    def forward(self, x):
        mean, var = tf.nn.moments(
            x, len(x.shape) - 1, shift=None, keepdims=True, name=None)
        std = tf.sqrt(var)
...
class SegmentEmbedding(object):
    def __init__(self, embed_size=512):
        self.embeds = tf.constant(np.random.random_sample(
            (3, embed_size)), dtype=tf.float32)
...
class BERTEmbedding(object):
    ...
    def forward(self, sequence, segment_label):
        x = self.token.forward(
            sequence) + self.position.forward(sequence) + self.segment.forward(segment_label)
        return x
...
class MultiHeadedAttention(object):
    ...
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.linear_layers = [lambda x: make_linear(
            x, d_model) for _ in range(3)]
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x, attn = self.attention.forward(
            query, key, value, mask=mask, dropout=0)

        # 3) "Concat" using a view and apply a final linear.
        # x = x.transpose(1, 2).contiguous().view(batch_size, -1, self.h * self.d_k)
        x = tf.reshape(tf.transpose(
            x, perm=[0, 2, 1, 3]), (batch_size, -1, self.h * self.d_k))
...
class BERTFULL(object):
    def __init__(self, hidden=256, n_layers=8, attn_heads=8, dropout=0.1):
        ...
        self.transformer_blocks = [TransformerBlock(
            hidden, attn_heads, hidden * 4, dropout) for _ in range(n_layers)]
