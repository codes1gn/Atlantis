...
from shared_functions import make_activation, make_conv2d, make_seperable_conv2d, make_avgpool2d, make_maxpool2d, measure_tf2_gpu, get_tf2_args
...
def normal_cell(prev, cur, out_channels):
    ...
    prev = fit(cur, prev)
    ts = list()
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels,
              kernels=(3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(cur)
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels,
              kernels=(3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels,
              kernels=(3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(make_avgpool2d(input_tensor=cur, kernels=(
        1, 1, 3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(prev)
    ts.append(make_avgpool2d(input_tensor=prev, kernels=(
        1, 1, 3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(make_avgpool2d(input_tensor=prev, kernels=(
        1, 1, 3, 3), strides=(1, 1, 1, 1), padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels,
...
def reduction_cell(prev, cur, out_channels):
    ...
    outputs = list()
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels,
              kernels=(7, 7), strides=(1, 1, 2, 2), padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=cur, out_channels=out_channels,
              kernels=(5, 5), strides=(1, 1, 2, 2), padding="SAME"))
    outputs.append(tf.add(ts[0], ts[1]))
    ts.append(make_maxpool2d(input_tensor=cur, kernels=(
        1, 1, 3, 3), strides=(1, 1, 2, 2), padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=prev, out_channels=out_channels,
              kernels=(7, 7), strides=(1, 1, 2, 2), padding="SAME"))
    ...
    ts.append(make_maxpool2d(input_tensor=cur, kernels=(
        1, 1, 3, 3), strides=(1, 1, 2, 2), padding="SAME"))
    ts.append(make_seperable_conv2d(input_tensor=outputs[0], out_channels=out_channels, kernels=(
        3, 3), strides=(1, 1, 1, 1), padding="SAME"))
