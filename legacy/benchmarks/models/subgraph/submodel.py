...
class SubModule1(torch.nn.Module):
    def __init__(self, in_filters=3, out_filters=3):
        ...
        self.bn = torch.nn.BatchNorm2d(out_filters)
        self.conv1 = torch.nn.Conv2d(in_filters, out_filters, (3, 3), padding=1)
        # self.conv2 = torch.nn.Conv2d(in_filters, out_filters, (1, 1), padding=1)
        self.conv3 = torch.nn.Conv2d(in_filters, out_filters, (3, 3), padding=1)

    def forward(self, x):
        out = self.bn(self.conv3(self.conv1(self.conv1(x))))
...
class SubModule3(torch.nn.Module):
    def __init__(self, in_filters=5, out_filters=5):
        super(SubModule3, self).__init__()
        self.bn = torch.nn.BatchNorm2d(out_filters)
        self.conv = torch.nn.Conv2d(in_filters, out_filters, (3, 3), padding=1)
