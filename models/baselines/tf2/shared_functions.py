from mortise.report.e2e_perf_logger import E2EPerfLogger
...
sys.path.insert(0, f'{this_code_path}/../../../../../report')
# print(sys.path)
...
def log_e2e_perf(args, method, mean_perf, std_perf, is_perf_logging):
    if is_perf_logging:
        E2EPerfLogger().log_perf(args.hardware, args.batch_size,
...
def make_conv2d(input_tensor, filter_shape, strides, padding, actimode, name):
    ...
    weights = tf.constant(np.random.random_sample(
        filter_shape), name=weights_name, dtype=tf.float32)

    if (input_tensor[2] == input_tensor[1]):
        conv2d = tf.nn.conv2d(input=input_tensor, filters=weights, strides=strides,
                              padding=padding, data_format="NHWC", name=conv_name)
    else:
        conv2d = tf.nn.conv2d(input=input_tensor, filters=weights, strides=strides,
                              padding=padding, data_format="NCHW", name=conv_name)
...
def make_conv3d(input_tensor, filter_shape, strides, padding, actimode, name):
    ...
    weights = tf.constant(np.random.random_sample(
        filter_shape), name=weights_name, dtype=tf.float32)
    conv2d = tf.nn.conv3d(input=input_tensor, filters=weights, strides=strides,
                          padding=padding, data_format="NCDHW", name=conv_name)
    return make_activation(conv2d, actimode, name)
...
def make_conv2d_bn(input_tensor, filter_shape, strides, padding, actimode, name):
    ...
    mean = tf.constant(np.random.random_sample(
        (1, conv2d.shape[1], 1, 1)), name=mean_name, dtype=tf.float32)
    variance = tf.constant(np.random.random_sample(
        (1, conv2d.shape[1], 1, 1)), name=variance_name, dtype=tf.float32)
    offset = tf.constant(np.random.random_sample(
        (1, conv2d.shape[1], 1, 1)), name=offset_name, dtype=tf.float32)
    scale = tf.constant(np.random.random_sample(
        (1, conv2d.shape[1], 1, 1)), name=scale_name, dtype=tf.float32)
    bn = tf.nn.batch_normalization(
        conv2d, mean, variance, offset, scale, 1e-7, name=name + "_bn")
...
def make_seperable_conv2d(input_tensor, out_channels, kernels, strides, padding, actimode="NONE", name="seperable_conv2d"):
    ...
    dp_filter = tf.constant(np.random.random_sample(
        depthwise_filter_shape), name=name+"_dp_filter", dtype=tf.float32)
    pw_filter = tf.constant(np.random.random_sample(
        pointwise_filter_shape), name=name+"_pw_filter", dtype=tf.float32)
    conv2d = tf.nn.separable_conv2d(input=input_tensor, depthwise_filter=dp_filter,
                                    pointwise_filter=pw_filter, strides=strides, padding=padding, data_format="NCHW", name=name)
    return make_activation(conv2d, actimode, name)
...
def make_linear(input_tensor, out_channels):
    ...
    weight = tf.constant(np.random.random_sample(
        weight_shape), dtype=tf.float32)
...
def tf2_args_checker(args, parser):
    ...
    is_missing_arg |= not args.hardware
...
def get_tf2_args():
    ...
    parser.add_argument("-hw", "--hardware", help="target hardware")
    parser.add_argument("-bs", "--batch-size", default=1,
                        type=int, help="batch size")

    # Measurement related parameters
    parser.add_argument(
        "--iterations", help="How many iterations to average for timing", type=int, default=50)
    parser.add_argument(
        "--discard_iter", help="How many iterations to not time during warm up", type=int, default=10)
...
def measure_tf2_gpu(model, inputs, method_name, args):
    ...
    # elif args.hardware in nvidia_gpus:
    elif args.hardware == "gpu":
        pass
    else:
        Exception(
            f"Unexpected HW for external compiler op pass: {args.hardware}")
    ...
    print(f"[{args.network}] Performance of {method_name} on {args.hardware} (mean, std) = ({mean_perf:.4f}+-{std_perf:.4f})")
