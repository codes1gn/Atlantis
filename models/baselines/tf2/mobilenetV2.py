...
def block(tensor, inp, oup, stride, expand_ratio):
    ...
    # print(inp)
    if expand_ratio == 1:
        convd = make_conv2d(input_tensor=convd, filter_shape=(3, 3, hidden_dim, hidden_dim), strides=(
            1, 1, stride, stride), padding=[[0, 0], [0, 0], [1, 1], [1, 1]], actimode="RELU", name="conv1")
        convd = make_conv2d(input_tensor=convd, filter_shape=(1, 1, hidden_dim, oup), strides=(
            1, 1, 1, 1), padding=[[0, 0], [0, 0], [0, 0], [0, 0]], actimode="NONE", name="conv1")
        ...
        convd = make_conv2d(input_tensor=convd, filter_shape=(1, 1, inp, hidden_dim), strides=(
            1, 1, 1, 1), padding=[[0, 0], [0, 0], [1, 1], [1, 1]], actimode="RELU", name="conv1")
        groups = hidden_dim
        t = tf.split(convd, groups, axis=1, name="split")
        assert (len(t) == groups)
        for i in range(groups):
            t[i] = make_conv2d(input_tensor=t[i], filter_shape=(3, 3, t[i].shape[1], hidden_dim//groups), strides=(
                1, 1, stride, stride), padding=[[0, 0], [0, 0], [1, 1], [1, 1]], actimode="RELU", name="conv2_".format(i))
...
def mobilenetv2_tf2_model(input):
    ...
    for t, c, n, s in inverted_residual_setting:
        ...
        for i in range(n):
            if i == 0:
                tensor = block(tensor, input_channel,
                               output_channel, s, expand_ratio=t)
            else:
                tensor = block(tensor, input_channel,
                               output_channel, 1, expand_ratio=t)
            input_channel = output_channel
    tensor = make_conv2d(input_tensor=tensor, filter_shape=(1, 1, input_channel, 1280), strides=(
        1, 1, 1, 1), padding="SAME", actimode="RELU", name="last_conv")
    return tensor
