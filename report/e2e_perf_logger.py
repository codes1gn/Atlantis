...
EXP_RESULT_PATH = f"{this_code_path}/../records"
E2E_PERF_LOG_PATH = f"{EXP_RESULT_PATH}/e2e_perf.csv"
E2E_PERF_COLS = ["HW", "BatchSize", "Network",
                 "Method", "Mean Perf", "Std Perf"]
...
class E2EPerfLogger:
        ...
        if os.path.isfile(E2E_PERF_LOG_PATH):
            with open(E2E_PERF_LOG_PATH, newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    key_val_dic[self.gen_dic_key(
                        hw, batch_size, net, method)] = (mean, std)
    ...
    def log_all_perf(self, memo_dic):
        ...
        for key, val in memo_dic.items():
            ...
            perf_str_row.append(
                f"{hw},{batch_size},{net},{method},{mean:.4f},{std:.4f}\n")

        perf_str_row.sort()

        with open(E2E_PERF_LOG_PATH, 'w') as e2e_log:
            # e2e_log.write(f"'hw', 'batch_size', 'network', 'backends', 'mean_perf', 'std_perf'\n")
...
AFUSION_PERF_LOG_PATH = f"{EXP_RESULT_PATH}/afusion_perf.csv"
AFUSION_PERF_COLS = ["func", "HW", "BatchSize",
                     "Network", "Method", "Mean Perf", "Compile Time"]


class AfusionPerfLogger:
    def __init__(self):
        pass

    def gen_dic_key(self, func, hw, batch_size, network, method):
        return (func, hw, batch_size, network, method)

    def read_dict_from_csv(self):
        key_val_dic = {}

        if os.path.isfile(AFUSION_PERF_LOG_PATH):
            with open(AFUSION_PERF_LOG_PATH, newline='') as csvfile:
                reader = csv.reader(csvfile, delimiter=',')
                for row in reader:
                    func, hw, batch_size, net, method, mean, compile = row
                    key_val_dic[self.gen_dic_key(
                        func, hw, batch_size, net, method)] = (mean, compile)
    ...
    def log_all_perf(self, memo_dic):
        perf_str_row = []
        for key, val in memo_dic.items():
            func, hw, batch_size, net, method = key
            mean = val[0]
            if (val[1] != 0):
                compile = val[1]
            perf_str_row.append(
                f"{func},{hw},{batch_size},{net},{method},{mean},{compile}\n")

        perf_str_row.sort()

        with open(AFUSION_PERF_LOG_PATH, 'w') as e2e_log:
            # e2e_log.write(f"'hw', 'batch_size', 'network', 'backends', 'mean_perf', 'std_perf'\n")
            for row in perf_str_row:
                e2e_log.write(row)

    def log_perf(self, func, hw, batch_size, network, method, mean_perf, compile):
        memo_dic = self.read_dict_from_csv()
        key = self.gen_dic_key(func, hw, batch_size, network, method)

        # Update performance if it already exists
        memo_dic[key] = (mean_perf, compile)
        self.log_all_perf(memo_dic)


DP_TUNING_TIME_LOG_PATH = f"{EXP_RESULT_PATH}/tuning_time.csv"
