...
import math
from reversyn.reversify.revop import InvertibleModuleWrapper, create_coupling
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, noactivation=False):
        ...
        self.basicblock_sub = BasicBlockSub(
            inplanes, planes, stride, noactivation)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, noactivation=False):
        ...
        self.bottleneck_sub = BottleneckSub(
            inplanes, planes, stride, noactivation)
        self.downsample = downsample
        self.stride = stride
...
class RevBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, noactivation=False):
        ...
        if downsample is None and stride == 1:
            gm = BasicBlockSub(inplanes // 2, planes //
                               2, stride, noactivation)
            fm = BasicBlockSub(inplanes // 2, planes //
                               2, stride, noactivation)
            coupling = create_coupling(Fm=fm, Gm=gm, coupling='additive')
            self.revblock = InvertibleModuleWrapper(
                fn=coupling, keep_input=False)
        else:
            self.basicblock_sub = BasicBlockSub(
                inplanes, planes, stride, noactivation)
        self.downsample = downsample
...
class RevBottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, noactivation=False):
        ...
        if downsample is None and stride == 1:
            gm = BottleneckSub(inplanes // 2, planes //
                               2, stride, noactivation)
            fm = BottleneckSub(inplanes // 2, planes //
                               2, stride, noactivation)
            coupling = create_coupling(Fm=fm, Gm=gm, coupling='additive')
            self.revblock = InvertibleModuleWrapper(
                fn=coupling, keep_input=False)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, channels_per_layer=None, strides=None,
        ...
        self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=init_kernel_size,
                               stride=strides[0], padding=(
                                   init_kernel_size - 1) // 2,
                               bias=False)
        ...
        self.layer1 = self._make_layer(
            block, channels_per_layer[1], layers[0], stride=strides[1], noactivation=True)
        self.layer2 = self._make_layer(
            block, channels_per_layer[2], layers[1], stride=strides[2])
        self.layer3 = self._make_layer(
            block, channels_per_layer[3], layers[2], stride=strides[3])
        self.has_4_layers = len(layers) >= 4
        if self.has_4_layers:
            self.layer4 = self._make_layer(
                block, channels_per_layer[4], layers[3], stride=strides[4])
        self.bn_final = batch_norm(self.inplanes)  # channels_per_layer[-1])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(
            channels_per_layer[-1] * block.expansion, num_classes)
...
# memcnn
# https://github.com/silvandeleemput/memcnn/blob/master/memcnn/models/tests/test_resnet.py
revnet50 = ResNet(
    RevBottleneck,
    [3, 4, 6, 3],
    num_classes=1000,
...
#     [2, 2, 11, 2],
#     num_classes=1000,
#     channels_per_layer=[128, 128, 256, 512, 832],
#     init_max_pool=True,
#     batch_norm_fix=False,
#     strides=[1, 1, 2, 2, 2],
#     # init_kernel_size=3
